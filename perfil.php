...
<?php
  ...
  $infouser = mysqli_query($conexion,"SELECT * FROM usuarios WHERE id_use = '$id'");
  $use = mysqli_fetch_array($infouser,MYSQL_ASSOC);

  $amigos = mysqli_query($conexion,"SELECT * FROM amigos WHERE de = '$id' AND para = '".$_SESSION['id']."' OR de = '".$_SESSION['id']."' AND para = '$id'");
  $ami = mysqli_fetch_array($amigos,MYSQL_ASSOC);
  ?>
<!DOCTYPE html>
<html>
<head>
  ...
  <title><?php echo $use['nombre']; ?> | Asystem</title>
          ...
          <div class="box box-primary">
            <div class="box-body box-profile">
              ...
              if(isset($_POST['seguir'])) {
                $add = mysqli_query($conexion,"INSERT INTO amigos (de,para,fecha,estado) values ('".$_SESSION['id']."','$id',now(),'0')");
              ...
              if(isset($_POST['seguirdirecto'])) {
                $add = mysqli_query($conexion,"INSERT INTO amigos (de,para,fecha,estado) values ('".$_SESSION['id']."','$id',now(),'1')");
                if($add) {echo '<script>window.location="perfil.php?id='.$id.'"</script>';}
              }
              ?>
              ...
              if(isset($_POST['dejarseguir'])) {
                $add = mysqli_query($conexion,"DELETE FROM amigos WHERE de = '$id' AND para = '".$_SESSION['id']."' OR de = '".$_SESSION['id']."' AND para = '$id'");
                if($add) {echo '<script>window.location="perfil.php?id='.$id.'"</script>';}
              }
              ?>
              
              <br>
              <a href="chat.php?usuario=<?php echo $id; ?>"><input type="button" class="btn btn-default btn-block" name="dejarsegui" value="Enviar chat"></a>
